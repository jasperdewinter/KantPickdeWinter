function [fcst Date_fcst P R_KF Q W C S] = RUN_DFM_ECB(X,y,Date,fcstH,P,now)
...
FQ(1:2,:) = nan;                                  			  % Set first two rows equal to nan, because the first to months do not form a complete month (in order for this to hold data must start at 1st month of a year!)

%_____________________________________________________
% 2. Set the Parameterset
if Q.crit  == 0;                                              % If Q.crit=0 than use the supplied # of static  & dynamic factors as well as the lag length in the VAR
    ParSet   = combvec(Q.r,Q.q,Q.p);                          % Combine the # of static, dynamic factors as well as the lag length in a matrix
...
% 4. Main loop
fcst  = [];
for j = 1:size(ParSet,2)                                      % Run the model for j=1 (j is always 1 in our case since size (ParSet,2) always equals 1)
    % Set parameters
    ...
    Q     = Estim_PC(Fj,Q,n);                                 % Find # of dynamic factors and matrix B and I(q) (Banbura and Rünstler, p. 3)
    Q.C   = [Vj zeros(n,Q.r*(Q.p-1))];                        % matrix of eigenvectors horizontally concatenated to zero matrix
    Q.R   = diag(diag(cov(Xe-Fj*Vj')));                       % diagonal covariance matrix of multivariate white noise term in eq (1) of Banbura and Rünstler, eq (3), p. 4)
...
%   R_KF     = KIF([Xf yf],Q.Model,Q);                              % Calculate full Kalman Gain and F matrices (for later use in weights)
    ...
    C.back       = Contributions([Xf yf],W{(now-3)},Q.Model,Q,(now-3));  % Calculate contributions to the "nowcast" using outcome of KFWeights
    C.now        = Contributions([Xf yf],W{now},Q.Model,Q,now);          % Calculate contributions to the "backcast" using outcome of KFWeights    
    C.q1         = Contributions([Xf yf],W{(now+3)},Q.Model,Q,(now+3));  % Calculate contributions to the "1Q ahead forecast" using outcome of KFWeights     
    C.q2         = Contributions([Xf yf],W{(now+6)},Q.Model,Q,(now+6));  % Calculate contributions to the "2Q ahead forecast" using outcome of KFWeights 
