...
[F V D]   = Estim_SPC(Xe,max(Q.r),Q.crit);       			% Calculate Static Principal Components on standardized & cleaned X's, F= estimated factors, V= eigenvectors
...
if Q.crit  == 0;                                            % Als Q.crit=0 dan geen zoekcriterium hanteren
    ...
    Q.qflag  = 2;                                           % In ESTIM_PC kies je voor Bai & Ngi (2005) criterium voor keuze # dynamische factoren
end
...
[Xf,Xfm,Xfs] = z01(Xf);                                     % Create mean and standard deviation to transform signal back                                                           % and standard
...
for j = 1:size(ParSet,2)                                    % Run the model for j=1 (j is always 1 in our case sinze size (ParSet,2) always equals 1)
    ...
    % Store forecast in quarterly format (only the third rows are entries,
    % and these rows contain the quarterly forecasts based on three months
    % of y's.
    fcst  = [fcst fcstj(end-(fcstH+1)*3:3:end,:)];                  % Store forecasts
    
    % Denormalize signal of non-y data
    temp         = R_KF.signal(:,1:end-1);                          % Create temporary variable
    mean         = NaN(size(temp,1),size(temp,2));
    stdev        = NaN(size(temp,1),size(temp,2));
    for k=1:size(temp,1)
        mean(k,:)   = Xfm;                                          % Create matrix of means
        stdev(k,:)  = Xfs;                                          % Create matrix of standard deviations                
    end
    
    R_KF.signald=R_KF.signal;                                       % Create signal with y_t in last column
    R_KF.signald(:,1:end-1)=(R_KF.signal(:,1:end-1).*stdev)+mean;   % Denormalize x's
end

Date_fcst = Date_f(end-(fcstH+1)*3:3:end,:);                        % Create (Quarterly) Dates for the forecasts
