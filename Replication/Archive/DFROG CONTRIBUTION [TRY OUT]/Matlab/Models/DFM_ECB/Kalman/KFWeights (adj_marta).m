...
% (Harvey 1989, pp.154)
% The weights are additive and consequently for the first term we can use 
% the weights for filtering and for the second term we can calculate them 
% recursively.
...
% They are stored in 3-dimensional matrix where j runs along 3rd dimension 
  ...
  for t=T-1:-1:1
     ...
     for j = 1:t
         ...
         wF        = WF{t}(:,:,j);
      % CHANGED\ADDED by Marta; 21/08/09
%        wS(:,:,j) = (eye(p)-Pstar(:,:,t)*S(t+1).T) * wF + Pstar(:,:,t)*wSold(:,:,j);
     wS(:,:,j) = (eye(p)-Pstar(:,:,t)*S(t).T) * wF + Pstar(:,:,t)*wSold(:,:,j);
     end
     for j=t+1:T
