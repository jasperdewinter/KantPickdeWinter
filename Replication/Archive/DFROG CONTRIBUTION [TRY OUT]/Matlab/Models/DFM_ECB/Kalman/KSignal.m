function [R S] = SEC(Ym,Model,Par,R,flag,S);
...
%        S          SSF of the model (built in DFM_ECB)
...
% Go  
  for t = 1:nobs
      ...
      if flag
         ...
         R.sign_P(t,:)   = sqrt(diag(S.Z * squeeze(R.PmT(t,:,:)) * S.Z')); % het squeeze commando hersteld de originele covariantiematrix. val(t,:,1), val(t,:,2) en val(t,:,3) onder elkaar. Omdat het een symmetrische matirx is maakt het niet uit dat je nu de rijen onder elkaar zet i.p.v. de kolommen naast elkaar. matrix P definieer je vervolgens als de standaarddeviatie van de hoofddiagonaal (de varianties)
      else
