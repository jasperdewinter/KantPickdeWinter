...
%______________________________________________________________________
  ...
  % in our case this is a <573 x 1> matrix
  ix = ~isnan(y);
  % in our case this is a <573 x 573> matrix
  e  = eye(size(y,1));
  % matrix needed to restore the matrixsizes of K and iF after the
  % shrinkage in the filter itsself.
  L  = e(:,ix);

  % reduced matrix y containing 133 obs
  y  =    y(ix);
  Z  =  S.Z(ix,:);  
  G  =  S.G(ix,:);
  c1 = S.c1(ix,:);
