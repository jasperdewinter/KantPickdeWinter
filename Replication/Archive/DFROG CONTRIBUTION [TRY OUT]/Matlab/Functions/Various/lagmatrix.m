...
%   $Revision: 1.7.4.1 $   $Date: 2003/05/08 21:45:34 $

if nargin ~= 2
    error('GARCH:lagmatrix:UnspecifiedInput' , ' Inputs ''X'' and ''Lags'' are both required.');
end
...
if prod(size(lags)) ~= length(lags)       % check for a non-vector.
   error('GARCH:lagmatrix:NonVectorLags' , ' ''Lags'' must be a vector.');
end

lags  =  lags(:);                         % Ensure a column vector.

if any(round(lags) - lags)
   error('GARCH:lagmatrix:NonIntegerLags' , ' All elements of ''Lags'' must be integers.')
end
