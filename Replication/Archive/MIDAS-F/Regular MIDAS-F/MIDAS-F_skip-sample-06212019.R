...
num_factors<- 1 
modelName  <- "Skip-sampled PCA Factor model"
LagOrder   <- 2
typeFlag   <- 0
...
start_time <- Sys.time()
  ...
  for (i in 1:(length(myFiles)/3)) {
    ...
    for (h in 1:length(HorizonSet)) {
      ...
      # ##################### MIDAS Regression ##############################
      y <- Y_q_shifted
      min_vector <- list(y,F_m1,F_m2,F_m3)
      min_length <- length(min_vector[[which.min(as.vector(c(length(y),length(F_m1),length(F_m2),length(F_m3))))]])
...
################### Horizon performance ############################
Fcsts_RMSE <- vector(mode= "list", 6)
for (z in 1:6) {
  FcstSave_RMSE <- rep(NA, 12)
  a <- 1
  for (b in 4:dim(FcstSave)[2]) {
    FcstSave_RMSE[a] <- RMSE(Fcsts[[z]][(4:(dim(FcstSave)[1]-6)),b],Fcsts[[z]][(4:(dim(FcstSave)[1]-6)),3])
    a<- a + 1
  }
