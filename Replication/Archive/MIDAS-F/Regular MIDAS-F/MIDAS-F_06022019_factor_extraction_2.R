  ...
  Fcsts_RMSE <- matrix(NA, dim(configurations)[2], 12)
...
#   
  ...
  ################### Horizon performance ############################
  for (p in 1:dim(configurations)[2]) {
    a<-1
    for (b in 4:dim(FcstSave)[2]) {
      FcstSave_RMSE[p,(b-3)] <- RMSE(FcstSave[(4:(dim(FcstSave)[1]-6)),b],FcstSave[(4:(dim(FcstSave)[1]-6)),3])
      a <- a + 1
    }
  }
  ...
  if (saveFlag == 1) {
    ...
    avg_performance <- rep(NA, 12)
  for (col in 1:12){
    avg_performance[col] <- mean(Fcsts_RMSE[,col][!is.na(Fcsts_RMSE[,col])])
  }
  round(avg_performance,2)[-1]
