...
# Load data 
setwd("C://Users//Dennis//Documents//Study//Thesis//DFROG//Dennis - Copy (2)//Factor extraction new methods//Regular PCA")
...
for (version in c(1,2,3,4,5,6,7,8)) {
  setwd("C://Users//Dennis//Documents//Study//Thesis//DFROG//Dennis - Copy (2)//Factor extraction new methods//Regular PCA")
  if (version == 1) {
    modelName  <- "Regular lags MIDAS-F U 2009M1 LagOrder 2"
    LagOrder   <- 2
    typeFlag   <- 0 
  } else if (version == 2) {
    modelName  <- "Regular lags MIDAS-F U 2009M1 LagOrder 5"
    LagOrder   <- 5
    typeFlag   <- 0
  } else if (version == 3) {
    modelName  <- "Regular lags MIDAS-F U 2009M1 LagOrder 8"
    LagOrder   <- 8
    typeFlag   <- 0
  } else if (version == 4) {
    modelName  <- "Regular lags MIDAS-F U 2009M1 LagOrder 11"
    LagOrder   <- 11
    typeFlag   <- 0
  } else if (version == 5) {
    modelName  <- "Regular lags MIDAS-F R 2009M1 LagOrder 2"
    LagOrder   <- 2
    typeFlag   <- 1
  } else if (version == 6) {
    modelName  <- "Regular lags MIDAS-F R 2009M1 LagOrder 5"
    LagOrder   <- 5
    typeFlag   <- 1
  }else if (version == 7) {
    modelName  <- "Regular lags MIDAS-F R 2009M1 LagOrder 8"
    LagOrder   <- 8
    typeFlag   <- 1
  } else {
    modelName  <- "Regular lags MIDAS-F R 2009M1 LagOrder 11"
    LagOrder   <- 11
    typeFlag   <- 1
  }
        ...
        # for (z in 1:6) {
          ...
          for (b in 4:dim(FcstSave)[2]-1) {
            FcstSave_RMSE[a] <- RMSE(Fcsts[[1]][(4:(dim(Fcsts[[1]])[1]-6)),b], Fcsts[[1]][(4:(dim(Fcsts[[1]])[1]-6)),3])
            a<- a + 1
          }
          FcstSave_RMSE[a] <- RMSE(Fcsts[[1]][(5:(dim(Fcsts[[1]])[1]-6)),15], Fcsts[[1]][(5:(dim(Fcsts[[1]])[1]-6)),3])
        ...
        if (saveFlag == 1) {
          ...
          setwd("C://Users//Dennis//Documents//Study//Thesis//Forecast results//Factor extraction methods//Regular PCA")
