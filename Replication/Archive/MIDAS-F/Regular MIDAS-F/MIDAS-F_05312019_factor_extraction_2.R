  ...
  # Load data 
  setwd("C://Users//Dennis//Documents//Study//Thesis//DFROG//Dennis - Copy (2)//Factor extraction test")
  ...
  LagOrder   <- 11                                                                                     # Contemporaneous lag is already included!
  HorizonSet <- c(-1,0,1,2)                                                                          # Horizon for MIDAS models (in quarters)
  saveFlag   <- 1     
  typeFlag   <- 0                                                                                    # 0: MIDAS-U; 1: MIDAS-R (Almon lag)
  ...
  # Factor configuartions
  configurations    <- readMat("Factor_configurations.mat")$ParSet.iteration
  Fcsts <- vector(mode = "list", length = dim(configurations)[2])
  Fcsts_RMSE <- Fcsts
  
  test <- Y_ref[22:132]
  
  #factor_set <- which(configurations[1,] > num_factors-1)
#############################################################################
  start_time <- Sys.time()
...
#   
    ...
    for (h in 1:length(HorizonSet)) {
      ...
      fcst_diff        <- quarter_diff(Y_available_last, fcst_moment)
       ...
       if (num_factors == 1) {
          ...
          } else if (num_factors == 6){
            data_midas <- list(Y_q_shifted,F_m1,F_m2,F_m3,F_m4,F_m5,F_m6)
          } else if (num_factors == 7) {
            data_midas <- list(Y_q_shifted,F_m1,F_m2,F_m3,F_m4,F_m5,F_m6, F_m7)
          } else if (num_factors == 8) {
            data_midas <- list(Y_q_shifted,F_m1,F_m2,F_m3,F_m4,F_m5,F_m6, F_m7, F_m8)
          } else if (num_factors == 9){
            data_midas <- list(Y_q_shifted,F_m1,F_m2,F_m3,F_m4,F_m5,F_m6, F_m7, F_m8, F_m9)
          } else if (num_factors == 10){
            data_midas <- list(Y_q_shifted,F_m1,F_m2,F_m3,F_m4,F_m5,F_m6, F_m7, F_m8, F_m9, F_m10)
          } else if (num_factors == 11){
            data_midas <- list(Y_q_shifted,F_m1,F_m2,F_m3,F_m4,F_m5,F_m6, F_m7, F_m8, F_m9, F_m10, F_m11)
          }   else {
            data_midas <- list(Y_q_shifted,F_m1,F_m2,F_m3,F_m4,F_m5,F_m6, F_m7, F_m8, F_m9, F_m10, F_m12)
          }
        ...
        if (typeFlag == 1) {
          ...
          if (num_factors == 1) {
            ...
            midas_fit <- midas_r(y ~ fmls(a, LagOrder, FreqRatio, nealmon) + fmls(b, LagOrder, 
                                                                                  FreqRatio, nealmon) + fmls(c, LagOrder,FreqRatio, nealmon)
                                 + fmls(d, LagOrder,FreqRatio, nealmon)+fmls(e, LagOrder,FreqRatio, nealmon) + fmls(f, LagOrder,FreqRatio, nealmon) + fmls(g, LagOrder,FreqRatio, nealmon),
                                 start = list(a = rep(0.5, num_param),
                                              b = rep(0.5, num_param),c = rep(0.5, num_param),d = rep(0.5, num_param), e = rep(0.5, num_param), f = rep(0.5, num_param), g = rep(0.5, num_param) ) )
          } else if (num_factors == 8) {
            a <- F_m1
            b <- F_m2
            c <- F_m3
            ...
            midas_fit <- midas_r(y ~ fmls(a, LagOrder, FreqRatio, nealmon) + fmls(b, LagOrder, 
                                                                                  FreqRatio, nealmon) + fmls(c, LagOrder,FreqRatio, nealmon)
                                 + fmls(d, LagOrder,FreqRatio, nealmon)+fmls(e, LagOrder,FreqRatio, nealmon) + fmls(f, LagOrder,FreqRatio, nealmon) + fmls(g, LagOrder,FreqRatio, nealmon) + fmls(h, LagOrder,FreqRatio, nealmon),
                                 start = list(a = rep(0.5, num_param),
                                              b = rep(0.5, num_param),c = rep(0.5, num_param),d = rep(0.5, num_param), e = rep(0.5, num_param), f = rep(0.5, num_param), g = rep(0.5, num_param), h = rep(0.5, num_param) ) )
          } else if (num_factors == 9) {
            a <- F_m1
            b <- F_m2
            c <- F_m3
            ...
            midas_fit <- midas_r(y ~ fmls(a, LagOrder, FreqRatio, nealmon) + fmls(b, LagOrder, 
                                                                                  FreqRatio, nealmon) + fmls(c, LagOrder,FreqRatio, nealmon)
                                 + fmls(d, LagOrder,FreqRatio, nealmon)+fmls(e, LagOrder,FreqRatio, nealmon) + fmls(f, LagOrder,FreqRatio, nealmon) + fmls(g, LagOrder,FreqRatio, nealmon) + fmls(h, LagOrder,FreqRatio, nealmon)+ fmls(i, LagOrder,FreqRatio, nealmon),
                                 start = list(a = rep(0.5, num_param),
                                              b = rep(0.5, num_param),c = rep(0.5, num_param),d = rep(0.5, num_param), e = rep(0.5, num_param), f = rep(0.5, num_param), g = rep(0.5, num_param), h = rep(0.5, num_param), i = rep(0.5, num_param) ) )
          } else if (num_factors == 10) {
            a <- F_m1
            b <- F_m2
            c <- F_m3
            ...
            midas_fit <- midas_r(y ~ fmls(a, LagOrder, FreqRatio, nealmon) + fmls(b, LagOrder, 
                                                                                  FreqRatio, nealmon) + fmls(c, LagOrder,FreqRatio, nealmon)
                                 + fmls(d, LagOrder,FreqRatio, nealmon)+fmls(e, LagOrder,FreqRatio, nealmon) + fmls(f, LagOrder,FreqRatio, nealmon) + fmls(g, LagOrder,FreqRatio, nealmon) + fmls(h, LagOrder,FreqRatio, nealmon)+ fmls(i, LagOrder,FreqRatio, nealmon) + fmls(j, LagOrder,FreqRatio, nealmon),
                                 start = list(a = rep(0.5, num_param),
                                              b = rep(0.5, num_param),c = rep(0.5, num_param),d = rep(0.5, num_param), e = rep(0.5, num_param), f = rep(0.5, num_param), g = rep(0.5, num_param), h = rep(0.5, num_param), i = rep(0.5, num_param),j = rep(0.5, num_param) ) )
          } else if (num_factors == 11) {
            a <- F_m1
            b <- F_m2
            c <- F_m3
            ...
            midas_fit <- midas_r(y ~ fmls(a, LagOrder, FreqRatio, nealmon) + fmls(b, LagOrder, 
                                                                                  FreqRatio, nealmon) + fmls(c, LagOrder,FreqRatio, nealmon)
                                 + fmls(d, LagOrder,FreqRatio, nealmon)+fmls(e, LagOrder,FreqRatio, nealmon) + fmls(f, LagOrder,FreqRatio, nealmon) + fmls(g, LagOrder,FreqRatio, nealmon) + fmls(h, LagOrder,FreqRatio, nealmon)+ fmls(i, LagOrder,FreqRatio, nealmon) + fmls(j, LagOrder,FreqRatio, nealmon)+ fmls(k, LagOrder,FreqRatio, nealmon),
                                 start = list(a = rep(0.5, num_param),
                                              b = rep(0.5, num_param),c = rep(0.5, num_param),d = rep(0.5, num_param), e = rep(0.5, num_param), f = rep(0.5, num_param), g = rep(0.5, num_param), h = rep(0.5, num_param), i = rep(0.5, num_param),j = rep(0.5, num_param),k = rep(0.5, num_param) ) )
          } else {
            a <- F_m1
            b <- F_m2
            c <- F_m3
            ...
            midas_fit <- midas_r(y ~ fmls(a, LagOrder, FreqRatio, nealmon) + fmls(b, LagOrder, 
                                                                                  FreqRatio, nealmon) + fmls(c, LagOrder,FreqRatio, nealmon)
                                 + fmls(d, LagOrder,FreqRatio, nealmon)+fmls(e, LagOrder,FreqRatio, nealmon) + fmls(f, LagOrder,FreqRatio, nealmon) + fmls(g, LagOrder,FreqRatio, nealmon) + fmls(h, LagOrder,FreqRatio, nealmon)+ fmls(i, LagOrder,FreqRatio, nealmon) + fmls(j, LagOrder,FreqRatio, nealmon)+ fmls(k, LagOrder,FreqRatio, nealmon) +fmls(l, LagOrder,FreqRatio, nealmon),
                                 start = list(a = rep(0.5, num_param),
                                              b = rep(0.5, num_param),c = rep(0.5, num_param),d = rep(0.5, num_param), e = rep(0.5, num_param), f = rep(0.5, num_param), g = rep(0.5, num_param), h = rep(0.5, num_param), i = rep(0.5, num_param),j = rep(0.5, num_param),k = rep(0.5, num_param),l = rep(0.5, num_param) ) )
          }
        ...
        Fcsts_RMSE[[config_nr]] <- FcstSave_RMSE
  ...
  # lo <- loess(y~x)                                                                                 # Polynomial fit
  # plot(x,y, main="MIDAS performance",
  #      xlab="Horizon", ylab="Average RMSE")
  # lines(predict(lo), col='red', lwd=2)
  # 
  # lm <- lm(y~x)                                                                                    # Linear fit
  # plot(x,y, main="MIDAS performance",
  #      xlab="Horizon", ylab="Average RMSE")
  # lines(predict(lm), col='red', lwd=2)
   
  ##################### Save results ###############################
  if (saveFlag == 1) {
    setwd("C://Users//Dennis//Documents//Study//Thesis//DFROG//Dennis - Copy (2)//MIDAS results")
    write.xlsx(Fcsts, file="fcst results U6F.xlsx", sheetName="Fcst Results", 
               col.names=TRUE, row.names=TRUE, append=FALSE)
    write.xlsx(Fcsts_RMSE, file ="fcst results RMSE U6F.xlsx", sheetName="Fcst Results RMSE", 
               col.names=TRUE, row.names=TRUE, append=FALSE)
  }
