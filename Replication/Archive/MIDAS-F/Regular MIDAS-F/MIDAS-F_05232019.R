  ...
  LagOrder   <- 1                                                                                     # Contemporaneous lag is already included!
  HorizonSet <- c(-1,0,1,2)                                                                          # Horizon for MIDAS models (in quarters)
  saveFlag   <- 0     
  typeFlag   <- 0                                                                                    # 0: MIDAS-U; 1: MIDAS-R (Almon lag)
  num_param  <- 3                                                                                    # Specify number of parameters in restricted form
...
# for (z in 1:dim(configurations)[2]) {
#   num_factors      <- configurations[1,z]
#   
  for (i in 1:length(myFiles)) {
    ...
    for (h in 1:length(HorizonSet)) {
      ...
      print(paste('Configuration', config_nr, 'File: ', myFiles[i],'and forecast horizon: ', Horizon))
...
# } # configurations loop
  ...
  # write.xlsx(Fcsts, file="fcst results.xlsx", sheetName="Fcst Results", 
  #            col.names=TRUE, row.names=TRUE, append=FALSE)
  # write.xlsx(Fcsts_RMSE, file ="fcst results RMSE.xlsx", sheetName="Fcst Results RMSE", 
  #            col.names=TRUE, row.names=TRUE, append=FALSE)
