...
num_factors<- 1 
modelName  <- "Skip-sampled PCA Factor model 1991M8"
LagOrder   <- 2
typeFlag   <- 0
...
start_time <- Sys.time()
  ...
  for (i in 2:(length(myFiles)/3)) {
    ...
    for (h in 1:length(HorizonSet)) {
      ...
      min_vector <- list(F_m1,F_m2,F_m3)
      min_length <- length(min_vector[[which.min(as.vector(c(length(F_m1),length(F_m2),length(F_m3))))]])
      F_m <- cbind(F_m1[1:min_length], F_m2[1:min_length],F_m3[1:min_length])
