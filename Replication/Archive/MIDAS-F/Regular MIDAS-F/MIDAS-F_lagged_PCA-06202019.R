...
num_factors<- 1 
modelName  <- "Extended PCA 4 lags MIDAS-F"
LagOrder   <- 2
typeFlag   <- 0
...
for (num_factors in 1:6) {
  ...
  for (i in 1:length(myFiles)) {
    ...
    for (h in 1:length(HorizonSet)) {
      ...
      print(paste('File: ', myFiles[i],'and forecast horizon: ', Horizon, 'and iteration:', num_factors))
