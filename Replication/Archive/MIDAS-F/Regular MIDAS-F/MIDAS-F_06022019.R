  ...
  # Load data 
  setwd("C://Users//Dennis//Documents//Study//Thesis//DFROG//Dennis - Copy (2)//MIDAS factors")
  ...
  for (iteration in c(1,2,3,4)) {
    setwd("C://Users//Dennis//Documents//Study//Thesis//DFROG//Dennis - Copy (2)//MIDAS factors")
  if (iteration == 1) {
  modelName  <- "MIDAS-F U 5 lags"
  LagOrder   <- 5 
  typeFlag   <- 0                                                                                    # 0: MIDAS-U; 1: MIDAS-R (Almon lag)
  } else if (iteration == 2){
    modelName  <- "MIDAS-F U 8 lags"
    LagOrder   <- 8 
    typeFlag   <- 0                                                                                    # 0: MIDAS-U; 1: MIDAS-R (Almon lag)
  }                                                                                                # Contemporaneous lag is already included!
    else if (iteration == 3){
      modelName  <- "MIDAS-F R 2 lags"
      LagOrder   <- 2 
      typeFlag   <- 1                                                                                    # 0: MIDAS-U; 1: MIDAS-R (Almon lag)
    }  else {
      modelName  <- "MIDAS-F R 5 lags"
      LagOrder   <- 5 
      typeFlag   <- 1   
    }
...
#   
  for (i in 1:length(myFiles)) {
   ...
   # if (config_nr %in% factor_set) {
    # if (config_nr == 6) {
    #num_factors      <- configurations[1,config_nr]
    # F_m             <- F_m[,1:num_factors]      
    F_m              <- F_m_full[,1]
    Y_date           <- data$Date.f
    ...
    for (h in 1:length(HorizonSet)) {
      ...
      print(paste('File: ', myFiles[i],'and forecast horizon: ', Horizon, 'and iteration:',iteration))
 ...
 # } # myFiles loop
  ...
  ################### Horizon performance ############################
    ...
    for (b in 4:dim(FcstSave)[2]) {
      Fcsts_RMSE[p,(b-3)] <- RMSE(Fcsts[[p]][(4:(dim(FcstSave)[1]-6)),b],Fcsts[[p]][(4:(dim(FcstSave)[1]-6)),3])
    }
  }
  ...
  } # iteration loop of begin program
