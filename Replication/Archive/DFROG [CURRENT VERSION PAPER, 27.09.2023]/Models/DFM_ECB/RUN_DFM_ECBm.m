function [fcst Date_fcst P R_KF] = RUN_DFM_ECBm(X,y,Date,fcstH,P)
%__________________________________________________________________________
% [fcst Date_fcst P R_KF] = RUN_DFM_ECBm(X,y,Date,fcstH,P)
%
% Produces estimates and forecasts for the ECB DFM with m-o-m rates
...
%  y        Quarterly GDP  growth(ext to monthly freq!)          (nobs x 1)
...
%  P        Structure of parameters, in this case
%             OC
...
% Static principal components
  ...
  FQ        = filter([1/3 2/3 1 2/3 1/3],1,F);
  FQ(1:4,:) = nan;
...
% Main loop
  ...
  for j = 1:size(ParSet,2)  
    ...
    % Estimate dynamic model
     ...
     [Q.beta Q.s] = OLS(y(5:end-P.cutE,:),FQ(5:end,1:Q.r),1,0);
