...
save_coeff <- vector(mode = "list", 3*length(k_set)*fcst_av_nr) # Preallocate coefficient list for last estimation sample
...
for (i in 1:length(myFiles)) {
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    print(paste('File: ', myFiles[i],'and forecast horizon: ', Horizon, 'k is ', k))
    ...
    R <- matrix(0, dim(est_sample)[2], k)
    select_X <- c(sort(sample(1:dim(est_sample)[2], k, replace=F)))

    y_est <- Y_q_shifted[(Y_available_nr-dim(est_sample)[1]+1):Y_available_nr]
    y_fcst <- rep(NA, fcst_diff)
    y_sample <- c(y_est,y_fcst)
    data_RS <- data.frame(y_sample, x_sample[,select_X])
                   
    formula_RS <- RS_formula_fun(select_X, 1)
    RS <- lm(formula = formula_RS, data = data_RS[1:length(y_est),])  
    
    if (i == length(myFiles)) {                                                             # Save last iteration coefficients
      save_coeff[[(k_loop-1)*fcst_av_nr*3+(fcst_loop-1)*3+1]] <- RS$coefficients  #every 1st element: coefficients
      save_coeff[[(k_loop-1)*fcst_av_nr*3+(fcst_loop-1)*3+2]] <- select_X  #every 1st element: coefficients
      save_coeff[[(k_loop-1)*fcst_av_nr*3+(fcst_loop-1)*3+3]] <- k                #every 3rd element: number of predictors
    }

    ######################## MIDASSO prediction #################
    Y_q_fcst <- predict(RS, newdata = data_RS[((length(y_est)+1):length(y_sample)),(2:(k+1))])
...
data_optimal_k <- data.frame(date = dates, k = save_k[,1])

p <- ggplot(data_optimal_k)  + geom_line(aes(y = k, x = dates)) +
...
p + labs(title = "Optimal subspace dimension (k)", x = "Years", y = "k") +
...
avg_coefficients <- read.xlsx("fcst avg coefficients opt k 3 30 RS 1000.xlsx",1)[,-1]
data_optimal_coefficients <- data.frame(coef = avg_coefficients, var_num = 1:252)
...
p <- p + labs(title = "Partial selection of variables", x = "Variable number", y = "Coefficient") + 
  theme(plot.title = element_text(hjust = 0.5, size = 26,face="bold"), axis.text=element_text(size=16),
        axis.title=element_text(size=20)) 
p

# 2. Save the plot to a pdf
ggsave("RSP_k7_coeff.pdf", width = 10, height = 5)

ggsave(name, width = 10, height = 5)

name <- "RSP k7 coeff.pdf"
p
dev.off()
