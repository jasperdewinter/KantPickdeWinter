...
train_ratio_set <- c(0.6,0.7,0.8,0.9) # Check performance for various training set ratios

# for (train_ratio in train_ratio_set) {

  train_ratio <- 0.7
    modelName  <- paste("Random Forest realign iterative", train_ratio)
    ...
    for (i in 200:length(myFiles)) {
          ...
          for (col in 1:dim(X_iterative)[2]){
            ...
            X_realign_it      <- realign(X_iterative[,-col])
            if (!is.null(X_realign_it[[2]])) {
              data_X_realign_it <- X_realign_it[[1]][-(1:X_realign_it[[2]]),]
            } else{
              data_X_realign_it <- X_realign_it[[1]]
            }
            
            while (NA_X != 0) {
              X_input <- dim(data_X_realign_it[(1:(dim(data_X_realign_it)[1]-1)),])[1]
              Y_input <- length(X_iterative[(1:(sum(!is.na(X_iterative[,col]))-1)),col])
              X_start <- 0
              Y_start <- 0
              if (X_input > Y_input) {
                X_start <- X_input - Y_input
              }  else {
                Y_start <- Y_input - X_input
              }
              
              ######################### ######################### ###########################
              est_sample <- data_X_realign_it[((1+X_start):(dim(data_X_realign_it)[1]-1)),]
              #Split est_sample in training and validation set to determine number of predictors at all splits
              train_set <- sample(1:nrow(est_sample),(floor((train_ratio)*nrow(est_sample))))
              y_est <- X_iterative[((1+Y_start):(sum(!is.na(X_iterative[,col]))-1)),col]
              est_data <- data.frame(y_est, 
                                     est_sample)
              ...
              ######################## MIDASSO prediction #################
               fcst_sample <- data_X_realign_it[dim(data_X_realign_it)[1],]
                fcst_sample           <- matrix(fcst_sample, 1, length(fcst_sample))
                colnames(fcst_sample) <- colnames(est_data[,-1])
                X_iterative[(length(X_iterative[,1])-NA_X+1),col] <- predict(RF, newdata = fcst_sample, mtry = mtry_opt, ntree=400)

                print(paste0('File: ', myFiles[i], "Regressor matrix column: ", col))
              ######################### ######################### ###########################
          ...
          for (h in 1:length(HorizonSet)) {
            ...
            print(paste('File: ', myFiles[i],'and forecast horizon: ', Horizon, ', training ratio: ', train_ratio))
            ...
            if (Horizon %in% c(1,2)) {                                                                     # If/else to properly read NA's at beginning/end of sample
              ...
              Y_available_nr   <- length(Y_q)
              fcst_diff        <- quarter_diff(Y_available_last, fcst_moment) 
            } else {
              Y_available_last <- Date[((num_y-sum(is.na(Y_q)))*FreqRatio),]
              Y_available_last <- c(Y_available_last[1],m2q(Y_available_last[2]))
              Y_available_nr   <- length(Y_q) - sum(is.na(Y_q))
              fcst_diff        <- quarter_diff(Y_available_last, fcst_moment) 
            }
            
            if (fcst_diff == 0) {
              Y_q <- Y_q[-length(Y_q)]
              Y_available_nr <- Y_available_nr - 1
              fcst_diff        <- 1
            }
            ...
            obs_surplus <- (Y_available_nr - dim(est_sample)[1])
            if (obs_surplus < 0 ) { est_sample <- est_sample[obs_surplus,] }
            
            #Split est_sample in training and validation set to determine number of predictors at all splits
            train_set <- sample(1:nrow(est_sample),(floor((train_ratio)*nrow(est_sample))))
            est_data <- data.frame(y_est = Y_q[(Y_available_nr-dim(est_sample)[1]+1):Y_available_nr], 
                                   est_sample)
            ...
            saveRDS(FcstSave, file=paste0("fcst results ragged edge solution", modelName, ".rds"))
        ...
        if (saveFlag == 1) {
          ...
          setwd("C:/Users/Dennis/Documents/Study/Thesis/Forecast results/Ragged Edge")
