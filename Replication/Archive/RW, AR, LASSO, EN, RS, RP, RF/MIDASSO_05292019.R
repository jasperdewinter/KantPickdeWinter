...
for (i in 1:length(myFiles)) {
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    if (is.null(dim(fcst_sample)[1])) {
      Y_q_fcst <- predict(MIDASSO_fit_cv, newx = rbind(fcst_sample,fcst_sample),s=MIDASSO_fit_cv$lambda.1se )                # s: lambda.min or lambda.1se (for 1 stand. dev. distance)
    } else {
      Y_q_fcst <- predict(MIDASSO_fit_cv, newx = fcst_sample,s=MIDASSO_fit_cv$lambda.1se )                                           
    }
...
as.vector(round(horizon_performance,2))
##################### Save results ###############################
if (saveFlag == 1) {
