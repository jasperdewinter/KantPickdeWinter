...
saveFlag   <- 0   
modelName  <- "Ridge realign SHAPLEY "
...
for (i in 300:length(myFiles)) {
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    MIDASSO_fit_cv <- cv.glmnet(est_sample, 
              Y_q_shifted[(Y_available_nr-dim(est_sample)[1]+1):Y_available_nr], alpha = 0)
    # MIDASSO_fit_cv <- cv.glmnet(est_sample, 
...
################################# Shapley values ###############################
library(iml)
test <- FcstSave[(100:114),4]
MIDASSO_fit_cv

Shapley$new(MIDASSO_fit_cv, test, 100)
model = Predictor$new(model = MIDASSO_fit_cv, data.frame(data_X_h[-(1:3),]), y = Y_ref)

Shapley$new(model, data.frame(test),100)
