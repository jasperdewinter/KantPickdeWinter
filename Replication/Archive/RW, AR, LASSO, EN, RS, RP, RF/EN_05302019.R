...
saveFlag   <- 0     
alpha_type <- 1                                                                                            # alpha = 1: LASSO; alpha = 0: Ridge
gamma      <- 2
...
for (i in 1:length(myFiles)) {
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    MIDASSO_fit_ridge_cv <- cv.glmnet(est_sample, 
                                ...
                                alpha = 0)
    best_ridge_coef <- as.numeric(coef(MIDASSO_fit_ridge_cv, s = MIDASSO_fit_ridge_cv$lambda.1se))[-1]
    
    MIDASSO_fit_cv <- cv.glmnet(est_sample,
              Y_q_shifted[(Y_available_nr-dim(est_sample)[1]+1):Y_available_nr],
              alpha = alpha_type, penalty.factor = (1 / (abs(best_ridge_coef)^gamma)) )
...
as.vector(round(horizon_performance,2))[-1]
##################### Save results ###############################
if (saveFlag == 1) {
