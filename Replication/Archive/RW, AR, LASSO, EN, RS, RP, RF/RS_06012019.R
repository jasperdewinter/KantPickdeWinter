...
start_time <- Sys.time()
save_results <- matrix(NA, 11, 7)
k_loop <- 1

for (k  in c(11,12,13,14,15,16,17) ) {
...
for (i in 1:length(myFiles)) {
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    formula_RS <- RS_formula_fun(select_X)
...
save_results[,k_loop] <- as.vector(round(horizon_performance,2))[-1]
k_loop <- k_loop + 1
} # k loop
...
####################### TO DO ############################
# Loop average over 1000 predictions, vary k and save for plot of k-dependence
