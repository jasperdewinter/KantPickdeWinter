...
for (i in 1:length(myFiles)) {
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    RS <- lm(formula = formula_RS, data = data_RS[1:length(y_est),])                                                                                          # 0: do not include intercept
...
for (fcst_distance in 1:12) {
  for (t in 1:dim(cumulative_RMSE)[1]) {
    save_k[t,fcst_distance] <- which.min( unlist(lapply(save_cumulative_results, function(x) x[t,fcst_distance])) )
  }
}
