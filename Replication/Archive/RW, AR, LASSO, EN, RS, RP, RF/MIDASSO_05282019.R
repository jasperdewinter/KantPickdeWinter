...
for (i in 1:length(myFiles)) {
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    ######################### MIDASSO regression ################
    # Cross-validation
    MIDASSO_fit <- cv.glmnet(est_sample, 
    ...
    if (is.null(dim(fcst_sample)[1])) {
      Y_q_fcst <- predict(MIDASSO_fit, newx = rbind(fcst_sample,fcst_sample),s=MIDASSO_fit$lambda.min )                # s: lambda.min or lambda.1se (for 1 stand. dev. distance)
    } else {
      Y_q_fcst <- predict(MIDASSO_fit, newx = fcst_sample,s=MIDASSO_fit$lambda.min )                                           
    }
...
#################### TO DO ##############################
# 1) forecast similarities: B = N = F1 = F2...?
...
p <- p + geom_errorbar(aes(x=lam, ymin=cv-sd, ymax=cv+sd ),width=0.005,color = "#AFAFB9")
p + labs(title = "Cross-validation LASSO", x = "Forecast distance", y = "Average RMSFE") + theme(legend.position = c(0.8, 0.1))+
  ...
  scale_x_continuous(name = "Log(t)", sec.axis = sec_axis(~./(-0.044) - 22, name = "Number of variables included"),
                     trans = 'reverse')


plot(MIDASSO_fit)
# 
# performance <- data.frame(cbind(horizon_performance_U1F, horizon_performance_R1F, 1:11))
# 
# p <- ggplot(performance, aes(V3)) + 
#   geom_point(aes(y = horizon_performance_U1F, colour = "MIDAS-U 1F")) + 
#   geom_point(aes(y = horizon_performance_R1F, colour = "MIDAS-R 1F")) +
#   geom_line(aes(y = horizon_performance_U1F, colour = "MIDAS-U 1F")) + 
#   geom_line(aes(y = horizon_performance_R1F, colour = "MIDAS-R 1F")) 
# 
# p + labs(title = "Learning curve MIDAS-F", x = "Forecast distance", y = "Average RMSFE")+
#   labs(color="Model specifications") + theme(legend.position = c(0.8, 0.1))+
#   theme(plot.title = element_text(hjust = 0.5, size = 26,face="bold")) + 
#   theme(axis.text=element_text(size=16),
#         axis.title=element_text(size=20)) +
#   theme(legend.title = element_text( size = 18),
