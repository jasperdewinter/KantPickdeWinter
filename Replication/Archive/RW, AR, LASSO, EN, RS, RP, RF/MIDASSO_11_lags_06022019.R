...
for (i in 1:length(myFiles)) {
  ...
  data_X <- matrix(NA, (dim(X)[1]/FreqRatio), ((LagOrder+1) * dim(X)[2]) )
  for (p in 1:dim(X)[2]) {
    col_id <- p - 1
    data_X[,((1+col_id*(LagOrder+1)):((LagOrder+1)*(col_id+1)))] <- fmls(X[,p],LagOrder,FreqRatio)
  }
