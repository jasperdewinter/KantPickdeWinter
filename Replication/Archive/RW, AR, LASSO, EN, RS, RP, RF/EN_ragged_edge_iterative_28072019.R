...
for (i in 250:length(myFiles)) {
  ...
  for (col in 1:dim(X_iterative)[2]){
    ...
    while (NA_X != 0) {
      ...
      EN_cv <- cva.glmnet(data_X_realign_it[((1+X_start):(dim(data_X_realign_it)[1]-1)),]
                          ,X_iterative[((1+Y_start):(sum(!is.na(X_iterative[,col]))-1)),col])
      ...
      EN <- glmnet(data_X_realign_it[((1+X_start):(dim(data_X_realign_it)[1]-1)),], 
                   X_iterative[((1+Y_start):(sum(!is.na(X_iterative[,col]))-1)),col], 
                   alpha = best.params$alpha, lambda = best.params$lambda_EN)
      
    
      X_iterative[(length(X_iterative[,1])-NA_X+1),col] <- predict(EN, 
                                                                   newx = rbind(data_X_realign_it[dim(data_X_realign_it)[1],], 
                                                                                data_X_realign_it[dim(data_X_realign_it)[1],]),
                                                                   s=best.params$lambda.min)[1] 
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    if (obs_surplus < 0) {
      EN_cv <- cva.glmnet(est_sample[obs_surplus,],Y_q_shifted[(Y_available_nr-dim(est_sample)[1]+1):Y_available_nr])
       } else {
    ...
    if (obs_surplus < 0) {
      EN <- glmnet(est_sample[obs_surplus,], 
                   Y_q_shifted[(Y_available_nr-dim(est_sample)[1]+1):Y_available_nr], 
                   alpha = best.params$alpha, lambda = best.params$lambda_EN)
    } else {
    ...
    saveRDS(FcstSave, paste0(modelName, ".rds"))
