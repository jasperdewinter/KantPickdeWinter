...
for (i in 1:length(myFiles)) {
  ...
  for (col in 1:dim(X_iterative)[2]){
    ...
    while (NA_X != 0) {
      ...
      if (X_input > Y_input) {
       ...
       X_CV   <- cv.glmnet(data_X_realign_it[((1+X_start):(dim(data_X_realign_it)[1]-1)),], 
                                                          X_iterative[((1+Y_start):(sum(!is.na(X_iterative[,col]))-1)),col], alpha = 1)

        X_iterative[(length(X_iterative[,1])-NA_X+1),col] <- predict(X_CV, 
  ...
  for (h in 1:length(HorizonSet)) {
    ...
    if (Horizon %in% c(1,2)) {                                                                     # If/else to properly read NA's at beginning/end of sample
      ...
      Y_available_last <- Date[((num_y-sum(is.na(Y_q_shifted)))*FreqRatio),]                          # months
      Y_available_last <- c(Y_available_last[1],m2q(Y_available_last[2]))                             # quarters
      Y_available_nr   <- num_y - sum(is.na(Y_q_shifted))
      fcst_diff        <- quarter_diff(Y_available_last, fcst_moment) 
    }
    ...
    if (obs_surplus < 0 ) {
      MIDASSO_fit_cv <- cv.glmnet(est_sample[obs_surplus,], 
